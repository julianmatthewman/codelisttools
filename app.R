...
browsers <- purrr::map(paths, \(x) {
  rio::import(x, colClasses = c("character"))
}) |>
        ...
        sidebarPanel(
          fluidRow(
            ...
            fluidRow(
              column(
                6,
                actionButton("set_prompt_review", "Review", width = "100%")
              ),
              column(
                6,
                actionButton("set_prompt_classify", "Classify", width = "100%")
              )
            ),
            textAreaInput(
              "extra_prompt",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$import_codebrowser, {
    ...
    res <- tryCatch(
      withCallingHandlers(
        rio::import(inFile$datapath, colClasses = c("character")),
        warning = function(w) {
          import_warnings <<- unique(c(import_warnings, conditionMessage(w)))
          invokeRestart("muffleWarning")
        }
      ),
      error = function(e) {
        showNotification(
          paste0("Import error: ", e$message),
          type = "error",
          duration = NULL
        )
        NULL
    ...
    if (!is.null(res)) {
      codebrowser$data <- res

      if (length(import_warnings) > 0) {
        lapply(import_warnings, function(msg) {
          showNotification(paste0("Import warning: ", msg), type = "warning", duration = NULL)
        })
      } else {
        showNotification("Browser imported successfully", type = "message", duration = 2)
      }
    }
  })
  ...
  observeEvent(input$classify, {
    ...
    # Classify the term
    showNotification(
      paste0("Classification started (", active_chat$get_model(), ")"),
      duration = NULL,
      type = "default",
