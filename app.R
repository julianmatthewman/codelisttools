...
ui <- fluidPage(
  navbarPage(
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          hr(),
          textAreaInput("searchterms", "Searchterms", "diabetes", resize = "vertical"),
    ...
    tabPanel(
      ...
      fluidRow(
        column(3, id = "withborder", h4("Terms"), verbatimTextOutput("wordfreqterms")),
        column(3, id = "withborder", h4("Monograms"), DT::dataTableOutput("monograms")),
        column(3, id = "withborder", h4("Bigrams"), DT::dataTableOutput("bigrams")),
        column(3, id = "withborder", h4("Trigrams"), DT::dataTableOutput("trigrams"))
      )
    ),
    # tabPanel(
    #   "Categorisation",
...
server <- function(input, output, session) {
  ...
  output$select_code_cols <- renderUI({
    selectInput("codecol", label = NULL, names(codebrowser$data), ifelse("CODE" %in% names(codebrowser$data), "CODE", names(codebrowser$data)[[1]]))
  })
  ...
  output$matchcolumn <- renderUI({
    selectInput("matchcolumn_in", label = "Match on", intersect(names(lefttable()), names(righttable())), NULL)
  })
  ...
  output$select_search_cols_word_freq <- renderUI({
    selectInput("wordfreqcol", "Select column with terms", names(cl_for_word_freq()), names(cl_for_word_freq())[[1]], multiple = FALSE)
  })
